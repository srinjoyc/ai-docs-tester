...
# Use ~/Library/Caches instead of /tmp — on macOS /tmp is a symlink to
# /private/tmp, which causes Node.js to find two React instances (via the
# symlink path vs the real path) and break SSR hooks.
CACHE_DIR="$HOME/Library/Caches/docs-eval/zerodev-base-v9"
